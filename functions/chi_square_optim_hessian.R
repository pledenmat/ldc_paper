...
chi_square_optim_hessian <- function(ab,params, observations, returnFit,confRT_name = "RTconf", binning = T, denominator = F, model = 1, hm_up = NULL, hm_low = NULL ){
  #First, generate predictions:
  drift <- params[8:length(params)]
  params <- c(params[1:7],ab)
...
chi_square_optim_hessian_DDM <- function(params, fixed, observations, returnFit,confRT_name = "RTconf", binning = T, denominator = F, model = 1, hm_up = NULL, hm_low = NULL ){
  #First, generate predictions:
  drift <- params[5:length(params)]
  params <- c(params[1:4],fixed)
  names(params) <- c('a','ter','alpha','beta','z','ntrials','sigma','dt','vratio')
