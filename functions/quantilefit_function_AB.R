...
ldc_notime <- function(a,b,data){
  return(( (1+data$resp)/2)*(1 /(1 + exp(  -a*data$evidence2 - b )))  + ((1-data$resp)/2)*(1 /(1 + exp(a*data$evidence2 - b)) ))
}
ldc_a_time <- function(a,b,data){
  return(( (1+data$resp)/2)*(1 /(1 + exp( (-a*data$evidence2/sqrt(data$rt2+.00001)) - b)))  + ((1-data$resp)/2)*(1 /(1 + exp((a*data$evidence2/sqrt(data$rt2+.00001)) - b)) ))
}
ldc_b_time <- function(a,b,data){
  return( ((1+data$resp)/2) * (1 /(1 + exp( -a*data$evidence2 - b/sqrt(data$rt2+.00001) )))  + ((1-data$resp)/2) * (1 /(1 + exp(a*data$evidence2 - b/sqrt(data$rt2+.00001)))) )
}
ldc_time <- function(a,b,c,data){
  return(( (1+data$resp)/2)*(1 /(1 + exp( (1/(data$rt2+.00001)^c)* (-a*data$evidence2 - b))))  + ((1-data$resp)/2)*(1 /(1 + exp((1/(data$rt2+.00001)^c)* (a*data$evidence2 - b))) ))
}
#' chi_square_optim
#'
...
#' @examples
#' 
chi_square_optim_AB <- function(params, observations, returnFit,confRT_name = "RTconf", 
                                binning = T, condition = c("minus","control","plus"),
                                time = "all"){
  ...
  for (d in drift) {
    predictions <- data.frame(DDM_with_confidence_slow_fullconfRT(v=d,a=params['a'],ter=params['ter'],z=params['z'],ntrials=params['ntrials']*dim(observations)[1]/2/length(drift)/length(condition),s=params['sigma'],dt=params['dt'],t2distribution=rep(observations[,confRT_name],times=params['ntrials']),postdriftmod=params['vratio']))
    predictionsneg <- data.frame(DDM_with_confidence_slow_fullconfRT(v=-d,a=params['a'],ter=params['ter'],z=params['z'],ntrials=params['ntrials']*dim(observations)[1]/2/length(drift)/length(condition),s=params['sigma'],dt=params['dt'],t2distribution=rep(observations[,confRT_name],times=params['ntrials']),postdriftmod=params['vratio']))
  ...
  predictions <- trial
  predictions <- predictions[rep(seq_len(nrow(predictions)), length(condition)), ]
  ...
  if (time=="all") {
    ...
    predictions$cj_1 <- ldc_a_time(params["alpha"],params["beta"],predictions)
  } else if (time=="b") {
    predictions$cj_1 <- ldc_b_time(params["alpha"],params["beta"],predictions)
  } else if (time=="none") {
    predictions$cj_1 <- ldc_notime(params["alpha"],params["beta"],predictions)
  }
...
chi_square_optim_AB_bfix <- function(params, observations, returnFit,confRT_name = "RTconf", 
                                     binning = T, condition = c("minus","control","plus"),
                                     time="all"){
  #First, generate predictions:
  ...
  for (i in 1:length(alphas)) {
    if (time=="all") {
      ...
      predictions[predictions$condition == condition[i],"cj_1"] <- ldc_a_time(alphas[i],params["beta"],predictions[predictions$condition == condition[i],])
    } else if (time=="b") {
      predictions[predictions$condition == condition[i],"cj_1"] <- ldc_b_time(alphas[i],params["beta"],predictions[predictions$condition == condition[i],])
    } else if (time=="none") {
      predictions[predictions$condition == condition[i],"cj_1"] <- ldc_notime(alphas[i],params["beta"],predictions[predictions$condition == condition[i],])  
    }
  }
...
chi_square_optim_AB_afix <- function(params, observations, returnFit,confRT_name = "RTconf", 
                                     binning = T, condition = c("minus","control","plus"),
                                     time="all"){
  #First, generate predictions:
  drift <- params[(length(params)-2):length(params)]
  ...
  for (i in 1:length(betas)) {
    if (time=="all") {
      ...
      predictions[predictions$condition == condition[i],"cj_1"] <- ldc_a_time(params["alpha"],betas[i],predictions[predictions$condition == condition[i],])
    } else if (time=="b") {
      predictions[predictions$condition == condition[i],"cj_1"] <- ldc_b_time(params["alpha"],betas[i],predictions[predictions$condition == condition[i],])
    } else if (time=="none") {
      predictions[predictions$condition == condition[i],"cj_1"] <- ldc_notime(params["alpha"],betas[i],predictions[predictions$condition == condition[i],])  
    }
  }
...
chi_square_optim_AB_nofix <- function(params, observations, returnFit,confRT_name = "RTconf", 
                                      binning = T, condition = c("minus","control","plus"),
                                      time="all"){
  #First, generate predictions:
  drift <- params[(length(params)-2):length(params)]
  ...
  for (i in 1:length(betas)) {
    if (time=="all") {
      ...
      predictions[predictions$condition == condition[i],"cj_1"] <- ldc_a_time(alphas[i],betas[i],predictions[predictions$condition == condition[i],])
    } else if (time=="b") {
      predictions[predictions$condition == condition[i],"cj_1"] <- ldc_b_time(alphas[i],betas[i],predictions[predictions$condition == condition[i],])
    } else if (time=="none") {
      predictions[predictions$condition == condition[i],"cj_1"] <- ldc_notime(alphas[i],betas[i],predictions[predictions$condition == condition[i],])  
    }
  }
...
chi_square_optim_AB_nofix_timefit <- function(params, observations, returnFit,confRT_name = "RTconf", 
                                              binning = T, condition = c("minus","control","plus")){
  #First, generate predictions:
  drift <- params[(length(params)-2):length(params)]
  betas <- params[(length(params)-5):(length(params)-3)]
  alphas <- params[(length(params)-8):(length(params)-6)]
  params <- params[1:(length(params)-9)]
  names(params) <- c('a','ter','z','ntrials','sigma','dt','vratio','gamma')
  ...
  for (i in 1:length(betas)) {
    predictions[predictions$condition == condition[i],"cj_1"] <- ldc_time(alphas[i],betas[i],params['gamma'],predictions[predictions$condition == condition[i],])
  }
